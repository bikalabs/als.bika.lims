...
Samples,    ClientSampleID,    SamplingDate,DateSampled,Sampler,SamplePoint,SampleMatrix,SampleType,ContainerType,ReportDryMatter,Priority,Total number of Analyses or Profiles,Price excl Tax,ECO,SAL,COL,TAS,MicroBio,Properties
Analysis price,,,,,,,,,,,,,,
...
"Sample 4", HHS14002,          3/9/2014,    3/9/2014,,Toilet,     Liquids,     Water,     Cup,          0,              Normal,  2,                                   0,             1,0,0,0,1,0
        ...
        states = [workflow.getInfoFor(a.getObject(), 'review_state')
                  for a in analyses]
        if states != ['sampled'] * 12:
            self.fail('Analysis states should all be sampled, but are not!')
...
"Sample 1", HHS14001,          3/9/2014,    3/9/2014,,,     ,     Water,     Cup,          0,              Normal,  1,                                   0,             0,0,0,0,0,1
    ...
    def test_LIMS_2488_added_sampler_valid(self):
        pc = getToolByName(self.portal, 'portal_catalog')
        workflow = getToolByName(self.portal, 'portal_workflow')
        arimport = self.addthing(self.client, 'ARImport')
        arimport.unmarkCreationFlag()
...
"Total Analyses or Profiles",,,,,,,,,,,,,9,,,
Total price excl Tax,,,,,,,,,,,,,,
"Sample 1", HHS14001,          3/9/2014,    3/9/2014 10:30:00,   test_sampler,Toilet,     Liquids,     Water,     Cup,          0,              Normal,  1,                                   0,             0,0,0,0,0,1
"Sample 2", HHS14002,          3/9/2014,    3/9/2014 10:30:00, test_sampler,   Toilet,     Liquids,     Water,     Cup,          0,              Normal,  2,                                   0,             0,0,0,0,1,1
"Sample 3", HHS14002,          3/9/2014,    3/9/2014 10:30:00, test_sampler,   Toilet,     Liquids,     Water,     Cup,          0,              Normal,  4,                                   0,             1,1,1,1,0,0
"Sample 4", HHS14002,          3/9/2014,    3/9/2014 10:30:00, test_sampler,   Toilet,     Liquids,     Water,     Cup,          0,              Normal,  2,                                   0,             1,0,0,0,1,0
        """)
        ...
        if l != 4:
            self.fail('4 Samples were not created!  We found %s' % l)
        bac = getToolByName(self.portal, 'bika_analysis_catalog')
        analyses = [a.getObject() for a in bac(portal_type='Analysis')]
        l = len(analyses)
        if l != 12:
            self.fail('12 Analysis not found! We found %s' % l)
        states = [workflow.getInfoFor(a, 'review_state')
                  for a in analyses]
        if states != ['sampled'] * 12:
            self.fail('Analysis states should all be sampled, but are not!')
        for a in analyses:
    ...
    def test_LIMS_2488_added_sampler_invalid(self):
        pc = getToolByName(self.portal, 'portal_catalog')
        workflow = getToolByName(self.portal, 'portal_workflow')
        arimport = self.addthing(self.client, 'ARImport')
        arimport.unmarkCreationFlag()
...
"Total Analyses or Profiles",,,,,,,,,,,,,9,,,
Total price excl Tax,,,,,,,,,,,,,,
"Sample 1", HHS14001,          3/9/2014,    3/9/2014 10:30:00,   invalid_sampler,Toilet,     Liquids,     Water,     Cup,          0,              Normal,  1,                                   0,             0,0,0,0,0,1
        """)
        ...
        l = len(ars)
        if l != 1:
            self.fail('1 AnalysisRequests were not created!  We found %s' % l)
        l = len(bc(portal_type='Sample'))
        if l != 1:
            self.fail('1 Samples were not created!  We found %s' % l)
        bac = getToolByName(self.portal, 'bika_analysis_catalog')
        analyses = [a.getObject() for a in bac(portal_type='Analysis')]
        l = len(analyses)
        if l != 2:
            self.fail('2 Analysis not found! We found %s' % l)
        states = [workflow.getInfoFor(a, 'review_state')
                  for a in analyses]
        if states != ['sampled'] * 2:
            self.fail('Analysis states should all be sampled, but are not!')
